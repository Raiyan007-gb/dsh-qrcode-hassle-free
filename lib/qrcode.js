/**
 * Vendored QR encoder, self-contained ESM with no runtime dependencies.
 *
 * This is the QR code generator from `qrcode-terminal` (Kazuhiko Arase's
 * `qrcode-generator`, MIT), collapsed from its original CJS file layout
 * (vendor/QRCode/{QRCode,QR8bitByte,QRUtil,QRPolynomial,QRRSBlock,QRBitBuffer,
 * QRErrorCorrectLevel,QRMode,QRMath,QRMaskPattern}) into this one module.
 *
 * Vendoring (rather than depending on `qrcode-terminal`) is deliberate: when
 * this bundle is installed as a `link:`/`file:` spec, a dependency's package
 * cannot be required from inside the checkout, so the QR step failed with
 * "Cannot find module 'qrcode-terminal/vendor/QRCode'". Encoding a QR matrix
 * needs no network, no native code, and no terminal output, so carrying the
 * ~11 KB MIT encoder in-tree deletes the only remaining runtime dependency and
 * makes `dsh plugin add/remove dsh-remote-tunnel-easy` dependency-free.
 *
 * Original header:
...
/**
 * Encode `text` into a QR matrix. Returns the module count and a count×count
 * grid of 0/1 cells (row-major) for the browser to render.
 * @param {string} text - payload to encode (8-bit byte mode).
 * @returns {{ count: number, modules: number[][] }}
 */
