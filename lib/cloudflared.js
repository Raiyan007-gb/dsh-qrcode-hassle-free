/**
 * cloudflared binary resolution for the qrcode-hassle-free bundle.
 *
...
export function cacheDir() {
  if (process.platform === 'win32') {
    ...
    return join(base, 'qrcode-hassle-free', 'cloudflared')
  }
...
export async function resolveCloudflared(config, onProgress = () => {}) {
  ...
  const work = tmpdir()
  const tmpAsset = join(work, `qrcode-hassle-free-${asset.file}-${process.pid}`)
  const tmpBin = join(work, `qrcode-hassle-free-cloudflared-${process.pid}`)
  ...
  try {
    ...
    if (asset.archive) {
      ...
      const extractDir = join(work, `qrcode-hassle-free-extract-${process.pid}`)
